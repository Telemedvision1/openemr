...
function payerPatient($patient_id)
{
    ...
    $insQry = "SELECT insd.*, ic.ins_type_code FROM (SELECT pid, provider FROM insurance_data WHERE type = 'primary' ORDER BY id DESC) insd " .
              "INNER JOIN  insurance_companies ic ON insd.provider = ic.id " .
              "WHERE insd.pid = ?";
    $insRes = sqlStatement($insQry, [$patient_id]);
    while ($insRow = sqlFetchArray($insRes)) {
        if ($insRow['ins_type_code'] == 8) {//Self Pay (Private Insurance)
...
function allListsPat($type, $patient_id, $from_date, $to_date)
{
    ...
    $diagRes = sqlStatement($diagQry, [$type, $patient_id, $from_date, $to_date]);
...
function allOrderMedsPat($patient_id, $from_date, $to_date)
{
    ...
    $medRes = sqlStatement($medQry, [$patient_id, $from_date, $to_date]);
...
function allProcPat(?string $proc_type = null, $patient_id = null, $from_date = null, $to_date = null)
{
    ...
    $procRes = sqlStatement($procQry, [$proc_type, $patient_id, $from_date, $to_date]);
...
function allVitalsPat($patient_id, $from_date, $to_date)
{
    ...
    $vitRes = sqlStatement($vitQry, [$patient_id, $from_date, $to_date]);
...
function allImmuPat($patient_id, $from_date, $to_date)
{
    ...
    $immRes = sqlStatement($immQry, [$patient_id, $from_date, $to_date]);
...
{
    $patientHistRow = sqlQuery("SELECT tobacco, date FROM history_data WHERE pid= ? ORDER BY id DESC LIMIT 1", [$patient_id]);

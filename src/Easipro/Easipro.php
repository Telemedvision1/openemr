...
class Easipro
{
    ...
    {
        $response = oeHttp::usingHeaders(['Authorization' => 'Basic ' . self::packageAuth()])->get(OEGlobalsBag::getInstance()->getString('easipro_server') . '/2014-01/Forms/.json');
        $data = $response->body();
        return $data;
    }
    ...
    {
        $response = oeHttp::usingHeaders(['Authorization' => 'Basic ' . self::packageAuth()])->get(OEGlobalsBag::getInstance()->getString('easipro_server') . '/2014-01/Assessments/' . $form_oid . '.json');
        $data = $response->body();
        return $data;
    }
    ...
    {
        $response = oeHttp::usingHeaders(['Authorization' => 'Basic ' . self::packageAuth()])->get(OEGlobalsBag::getInstance()->getString('easipro_server') . '/2014-01/Participants/' . $assessment_oid . '.json');
        $data = $response->body();
        return $data;
    }
    ...
    public static function selectResponse($assessment_oid, $itemresponse_oid, $response)
    {
        ...
        //$query = "ItemResponseOID=" . $itemresponse_oid . "&Response=" . $response;
        $response = oeHttp::usingHeaders(['Authorization' => 'Basic ' . self::packageAuth()])->get(OEGlobalsBag::getInstance()->getString('easipro_server') . '/2014-01/Participants/' . $assessment_oid . '.json', $query);
        $data = $response->body();
        return $data;
    }
    ...
    {
        $response = oeHttp::usingHeaders(['Authorization' => 'Basic ' . self::packageAuth()])->get(OEGlobalsBag::getInstance()->getString('easipro_server') . '/2014-01/Results/' . $assessment_oid . '.json');
        $data = $response->body();
        return $data;
    }
    ...
    public static function requestAssessment($patient_id, $user_id, $form_oid, $form_name, $expiration, $assessment_oid, $status)
    {
        ...
        $message .= '<center>Go to: ' . text(OEGlobalsBag::getInstance()->getString('portal_onsite_two_address')) . '</center><br>Thanks.';
